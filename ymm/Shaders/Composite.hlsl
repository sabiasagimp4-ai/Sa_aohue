...
{
    if (!inGamut(rgb) || (brightness == 1 && (amount <= 0 || mask <= 0))) return rgb;
    float3 lab = toLab(isLinear > .5 ? rgb : decodeSrgb(max(rgb, 0)));
    lab.x = saturate(lab.x * brightness);
    float gain = 1 + amount * mask;
    float3 target = fromLab(float3(lab.x, lab.yz * gain));
    float lo = gain, hi = gain;
    if (!inGamut(target))
    {
        lo = 0;
        [unroll] for (int i = 0; i < 12; ++i) { float g = (lo + hi) * .5; if (inGamut(fromLab(float3(lab.x, lab.yz * g)))) lo = g; else hi = g; }
    }
    float3 result = fromLab(float3(lab.x, lab.yz * lo));
...
D2D_PS_ENTRY(main)
{
    ...
    if (outputMode < .5 && (source.a <= 0 || !inGamut(unpremultiply(source)) || (brightness == 1 && amount <= 0))) return source;
    float4 blurred = D2DSampleInputAtPosition(1, p);
    float mask = pow(saturate(blurred.a > 0 ? blurred.r / blurred.a : 0), 1.0 / max(.1, contrast));
    if (outputMode < .5 && mask <= 0 && brightness == 1) return source;
