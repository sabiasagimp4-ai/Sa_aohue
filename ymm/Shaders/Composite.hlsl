...
float3 adjustColor(float3 rgb, float mask, float spillMask, float4 chromaField)
{
    if (!inGamut(rgb) || (mask <= 0 && (abs(colorBleed) <= 1e-5 || spillMask <= 0))) return rgb;
    float3 lab = toLab(decodeSrgb(max(rgb, 0)));

    // The numeric reference applies the neighbouring OKLab chroma difference
    // before the ordinary line-guided colour controls.  The field stores
    // (a * alpha, b * alpha, alpha, 1), so division by its third channel gives
    // the locally weighted average chroma.  Positive values borrow colour;
    // negative values push the two sides farther apart.
    if (abs(colorBleed) > 1e-5 && spillMask > 0 && chromaField.b > 1e-5)
    {
        float2 neighbour = chromaField.rg / chromaField.b;
        lab.yz += (neighbour - lab.yz) * colorBleed * spillMask;
    }
    ...
    float gain = max(0, 1 + amount * mask);
    ...
    return encodeSrgb(saturate(result));
}
D2D_PS_ENTRY(main)
{
    float2 p = D2DGetScenePosition().xy;
    float4 source = D2DSampleInputAtPosition(0, p);
    if (outputMode < .5 && (source.a <= 0 || !inGamut(unpremultiply(source)) || (brightness == 1 && amount == 0 && hueShift == 0 && colorBleed == 0))) return source;
    float4 blurred = D2DSampleInputAtPosition(1, p);
    float mask = saturate(blurred.a > 0 ? blurred.r / blurred.a : 0);
    float4 clean = D2DSampleInputAtPosition(3, p);
    float spillMask = saturate(clean.a > 0 ? clean.r / clean.a : 0);
    ...
    if (sideMode > .5 && blurred.r > 0 && source.a > 0)
    {
        ...
        float sideWeight = SideWeight(sourceLuma, referenceLuma, sideMode);
        mask *= sideWeight;
        spillMask *= sideWeight;
    }
    if (outputMode < .5 && mask <= 0 && (abs(colorBleed) <= 1e-5 || spillMask <= 0)) return source;
    ...
    float4 chromaField = D2DSampleInputAtPosition(2, p);
    float3 rgb = adjustColor(unpremultiply(source), mask, spillMask, chromaField);
