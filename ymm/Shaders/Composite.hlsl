...
float3 increaseChroma(float3 rgb, float mask)
{
    if (amount <= 0 || mask <= 0 || !inGamut(rgb)) return rgb;
    float3 lab = toLab(isLinear > .5 ? rgb : decodeSrgb(max(rgb, 0)));
    float lo = 1, hi = 1 + amount * mask;
...
D2D_PS_ENTRY(main)
{
    ...
    if (outputMode < .5 && (amount <= 0 || source.a <= 0 || !inGamut(unpremultiply(source)))) return source;
    ...
    float3 rgb = increaseChroma(unpremultiply(source), mask);
