...
foreach ($name in @('GaussianHorizontal', 'LineMask', 'Composite')) {
    ...
    $layout = switch ($name) {
        ...
        'LineMask' { @(@('threshold', 0), @('invertLines', 4), @('scale', 8), @('positiveLobe', 12), @('smallInverseSum', 16), @('largeInverseSum', 20), @('inputBounds', 32)) }
        'Composite' { @(@('amount', 0), @('contrast', 4), @('outputMode', 8), @('brightness', 12), @('hueShift', 16)) }
    }
